...
def _stats_dene(full_payload, parse_offset, callback):
    """
    Protocol 18: Stats verisi EventData code=1 olarak geliyor.
    Guild adini parametrelerden bularak tespit eder.
    """
    if GUILD_BYTES not in full_payload:
        return
...
def _msg_isle(msg_type, payload, callback):
    ...
    try:
        if msg_type == MSG_EVENT_DATA:
            if len(payload) > 20:
                _stats_dene(payload, 1, callback)
            ...
            if ret_code == 0 and len(payload) > 50:
                _stats_dene(payload, 3, callback)

    except Exception as e:
        print(f"[HATA] Paket isleme: {e}")
